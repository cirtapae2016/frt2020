...
namespace pluginCalidadAprobar
{
...

    {
        ...
        internal static void ItemEventHandler(string formUID, ref ItemEvent oItemEvent, SAPbobsCOM.Company sbo_company, ref Application sbo_application, out bool bBubbleEvent, string sessionId)
        {
            ...
            switch (oItemEvent.ItemUID)
            {
                ...
                case pluginForm.ButtonAprove:
                    ButtonAprove(formUID, ref oItemEvent, sbo_company, ref sbo_application, out bBubbleEvent, sessionId);
                    break;
            }
        }
            ...
            if (oItemEvent.EventType == BoEventTypes.et_CLICK)
            {
                ...
                if (!oItemEvent.BeforeAction)
                {
                    try
                    {
                        ...
                        if (Todos == "N" || string.IsNullOrEmpty(Todos))
                        {
                            Where += "Where ifnull(T0.\"U_Revisado\",'N') = 'N'";
                        }
                        else if (Todos == "Y")
                        {
                            Where += "Where ifnull(T0.\"U_Revisado\",'N') in ('N','Y')";
                        }

                        if (!string.IsNullOrEmpty(Fecha))
                        {
                            Where += $" and T0.\"CreateDate\" = '{Fecha}'";
                        }

                        if (!string.IsNullOrEmpty(Proceso))
                        {
                            Where += $" and T0.\"U_PuntoControl\" like '%{Proceso}%'";
                        }
...
";
        ...
        private static void ButtonAprove(string formUID, ref ItemEvent oItemEvent, SAPbobsCOM.Company sbo_company, ref Application sbo_application, out bool bBubbleEvent, string sessionId)
        {
            bBubbleEvent = true;
            ...
            if (!oItemEvent.BeforeAction)
            {
                ...
                if (oItemEvent.EventType == BoEventTypes.et_CLICK)
                {
                    try
                    {
                        oForm.Freeze(true);
                        var grid = oForm.Items.Item(pluginForm.GridCalidad).Specific as Grid;
                        var Code = SAPFunctions.GetFieldFromSelectedRow(grid, "Registro");
                        var DocEntry = SAPFunctions.GetFieldFromSelectedRow(grid, "Correlativo");
                        ...
                        RegistroCalidad.U_Revisado = "Y";
                        RegistroCalidad.U_RevisadoPor = sbo_application.Company.UserName;

                        var message = CommonFunctions.PATCH(ServiceLayer.RegistroCalidad, RegistroCalidad, DocEntry, sessionId, out statusCode);
                        if (statusCode == System.Net.HttpStatusCode.NoContent)
                        {
                            sbo_application.MessageBox("Registro aprobado");
                        }
                        else
                        {
                            var _Error = message.DeserializeJsonToDynamic();
                            throw new Exception($"Error en la aprobacion : {_Error.error.message.value.ToString()}");
                        }
                    }
                    catch { throw; }
                    finally { oForm.Freeze(false); }
